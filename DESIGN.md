...
납부/차액 안내서는 기존 과목 색 토큰을 옅은 카드 표면과 왼쪽 상태선에 재사용한다. 수업별 금액에는 `회당 단가 × 횟수 · 1회당 시간`을 함께 표시하고, 원본 Access 합계와 계산 합계의 일치 여부는 색상뿐 아니라 텍스트로도 표시한다. 편집 표의 수치와 산출 기준은 `tabular-nums`를 유지하며 삭제 버튼은 40px 클릭 영역을 사용한다.
